                                ...
                                {users.map(({ id, firstname, lastname, age, country, recidency_status, ethnic_origin, profile_image, highest_education, current_profession, prayer_frequency, sect  }) => (
                                    <div key={id} className="flex-none sm:flex bg-white shadow-md sm:rounded-lg  space-y-2 mb-4">
                                        <div className="blur-[2px] overflow-hidden relative sm:min-h-full w-full sm:w-[19rem] sm:mb-0 mb-3">
                                            <img src={`http://localhost:3000/uploads/user-images/${profile_image}`} alt={`${firstname}'s Profile photo`}  className="blur-[2px] w-full sm:w-[19rem] h-auto sm:min-h-full inset-0 object-cover aspect-square sm:rounded-l-lg" />
                                        </div>
...
{/* {users.map(({ id, firstname, lastname, age, country, recidency_status, ethnic_origin, profile_image, highest_education, current_profession,  }) => (
        <div key={id} className="flex font-sans bg-white shadow-md rounded-lg space-y-2 mb-4">
            <div className="flex-none w-48 relative">
                <img src={`http://localhost:3000/uploads/user-images/${profile_image}`} alt={`${firstname}'s Profile photo`} className="absolute inset-0 w-full h-full object-cover rounded-l-lg" />
            </div>
